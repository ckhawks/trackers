...
export async function addProgressToTracker(prevState: any, formData: FormData) {
    ...
    if (formData.get("progress-userid") != session.user.id) {
        return { message: "You somehow are a differnt user than your session. Wtf?" };
    }
    ...
    if ((formData.get("progress-summary")?.toString() || "").length > 100) {
        return { message: "Please reduce the length of the description to less than 100 characters." };
    }
    ...
    const query2 = 'INSERT INTO "Progress" (userid, trackerid) VALUES ($1, $2) RETURNING id;';
    ...
    const query3 = 'INSERT INTO "ProgressEvent" (progressid, points, summary) VALUES ($1, $2, $3);';
    const params3 = [progressId, formData.get("progress-points"), formData.get("progress-summary")];
